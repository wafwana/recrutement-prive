...
async function main() {
  ...
  try {
    ...
    console.log("9. Testing Job Creation & Taxonomy Validation Server Rules...");
    const parentCategory = await prisma.jobCategory.upsert({
      where: { code: `FINANCE_${suffix}` },
      update: { isActive: true },
      create: { code: `FINANCE_${suffix}`, name: { fr: "Finance", en: "Finance" }, isActive: true },
    });
    const subCategory = await prisma.jobCategory.upsert({
      where: { code: `CONTROLE_${suffix}` },
      update: { isActive: true },
      create: { code: `CONTROLE_${suffix}`, name: { fr: "Contrôle de gestion", en: "Controlling" }, parentId: parentCategory.id, isActive: true },
    });

    const otherParentCategory = await prisma.jobCategory.upsert({
      where: { code: `RH_${suffix}` },
      update: { isActive: true },
      create: { code: `RH_${suffix}`, name: { fr: "Ressources Humaines", en: "HR" }, isActive: true },
    });

    const inactiveCategory = await prisma.jobCategory.upsert({
      where: { code: `INACTIVE_${suffix}` },
      update: { isActive: false },
      create: { code: `INACTIVE_${suffix}`, name: { fr: "Métier Inactif", en: "Inactive Category" }, isActive: false },
    });

    createdCategoryIds.push(parentCategory.id, subCategory.id, otherParentCategory.id, inactiveCategory.id);
    ...
    // Validation 9.1: Inactive category rejection rule
    const inactiveCheck = await prisma.jobCategory.findUnique({ where: { id: inactiveCategory.id } });
    assert(inactiveCheck?.isActive === false, "Inactive category should have isActive === false");

    // Validation 9.2: Non-existent category ID check
    const nonExistentCategory = await prisma.jobCategory.findUnique({ where: { id: "non_existent_cat_id_xyz" } });
    assert(nonExistentCategory === null, "Non-existent category should return null");

    // Validation 9.3: Subcategory belonging to another parent category check
    const mismatchSubCheck = await prisma.jobCategory.findFirst({
      where: { id: subCategory.id, parentId: otherParentCategory.id },
    });
    assert(mismatchSubCheck === null, "Subcategory must not belong to mismatched parent category");
