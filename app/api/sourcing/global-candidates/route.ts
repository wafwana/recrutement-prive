...
export async function POST(request: Request) {
  ...
  let candidates;
  try { candidates = await fetchGlobalCandidates(value.sourceUrl); }
  ...
  for (const candidate of candidates) {
    ...
    const result = matchCandidateToJob({
      ...
      requiredSkills: job.requiredSkills, requiredExperienceYears: job.requiredExperienceYears, title: job.title,
      description: job.description, location: job.location, categoryCode: job.jobCategory?.code, subCategoryCode: job.subCategory?.code,
    });

    const data = {
      source: candidate.source, sourceProfileUrl: candidate.sourceProfileUrl, name: candidate.name, headline: candidate.headline,
      location: candidate.location, skills: candidate.skills, experienceYears: candidate.experienceYears,
      status: "MATCHED" as const, matchingScore: result.score,
      matchingDetails: { jobId: job.id, externalId: candidate.externalId, match: result },
  ...
  await prisma.auditLog.create({
    data: {
      actorUserId: actor, actorRole: "SYSTEM", action: "GLOBAL_CANDIDATE_SOURCING", targetType: "JOB", targetId: job.id,
