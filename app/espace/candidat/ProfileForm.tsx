...
export default function ProfileForm({ profile }: { profile: Profile | null }) {
  ...
  return (
    <form action={formAction} className="border border-white/10 bg-[#111] p-8">
      ...
      <label className="mt-5 block text-xs uppercase tracking-[0.18em] text-white/40">Présentation<textarea name="bio" defaultValue={profile?.bio ?? ""} rows={6} maxLength={2000} className="mt-2 w-full resize-none border border-white/10 bg-transparent px-4 py-3 text-sm leading-6 text-white outline-none" /></label>
      <label className="mt-5 block text-xs uppercase tracking-[0.18em] text-white/40">Préférences de recherche<input name="preferences" defaultValue={preferences} placeholder="Direction, Finance, Paris" maxLength={1000} className="mt-2 w-full border border-white/10 bg-transparent px-4 py-3 text-sm text-white outline-none" /></label>
