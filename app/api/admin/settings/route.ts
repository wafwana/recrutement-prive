...
const settingsSchema = z.object({
  maintenanceMode: z.boolean(), allowCandidateRegistration: z.boolean(), allowCompanyRegistration: z.boolean(),
  notificationsEnabled: z.boolean(), defaultApplicationStatus: z.enum(["SUBMITTED", "REVIEWING", "INTERVIEW", "SHORTLISTED", "REJECTED", "HIRED"]),
});
const defaults = { maintenanceMode: false, allowCandidateRegistration: true, allowCompanyRegistration: true, notificationsEnabled: true, defaultApplicationStatus: "SUBMITTED" as const };
async function requirePlatformControl() {
  const session = await auth();
  if (!session?.user?.id || (session.user.role !== "ADMIN" && session.user.role !== "OWNER")) return null;
  if (!(await hasPermission(session.user.id, session.user.role, "PLATFORM_SETTINGS"))) return null;
...
export async function PUT(request: Request) {
  ...
  const record = await prisma.systemSetting.upsert({ where: { key: "platform" }, update: { value: parsed.data }, create: { key: "platform", value: parsed.data } });
