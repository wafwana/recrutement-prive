...
export async function saveCandidateProfile(formData: FormData) {
  ...
  if (parsed.data.primaryCategoryId) {
    ...
    throw new Error("Les sous-catégories nécessitent la sélection d'un métier principal.");
  }
  ...
  if (subCategoryIds.length > 0) {
    ...
    if (validSubCats.length !== subCategoryIds.length) {
      throw new Error("Une ou plusieurs sous-catégories sélectionnées sont invalides, inactives ou non rattachées au métier principal.");
    }
  }
