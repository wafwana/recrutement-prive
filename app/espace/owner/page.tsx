    ...
    <section className="mx-auto w-[min(1280px,calc(100%-40px))] py-12 md:w-[min(1280px,calc(100%-72px))] md:py-20">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link href="/espace/owner/entreprises/nouvelle" className="border border-[#c7a15a] bg-[#c7a15a]/10 px-4 py-3 text-[10px] uppercase tracking-[0.18em] text-[#c7a15a]">Rentrer une entreprise</Link>
          <Link href="/espace/owner/offres/nouvelle" className="border border-[#F97316] bg-[#F97316]/10 px-4 py-3 text-[10px] uppercase tracking-[0.18em] text-[#F97316]">Rentrer une offre</Link>
          <Link href="/espace/owner/offres-vivier" className="border border-[#c7a15a] bg-[#c7a15a]/10 px-4 py-3 text-[10px] uppercase tracking-[0.18em] text-[#c7a15a]">Vivier des offres</Link>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_.9fr]">
        <section className="border border-white/10 p-7">
          ...
          <div className="mt-7 space-y-3">
            {[["Espace candidat","Disponible"],["Espace entreprise","Disponible"],["Espace consultant","Disponible"],["Administration","Disponible"],["Matching explicable","Intégré"],["Sourcing","Intégré"],["Rôle Owner","Intégré"],["Prestations & tarifs","Modifiable"],["Automatisations avancées","À poursuivre"]].map(([label,status]) => (
              <div key={label} className="flex items-center justify-between gap-4 border border-white/10 px-4 py-3"><span className="text-sm text-white/75">{label}</span><span className="text-[10px] uppercase tracking-[0.15em] text-white/40">{status}</span></div>
            ))}
          </div>
        ...
        <section className="border border-white/10 p-7">
          <p className="text-[10px] uppercase tracking-[0.25em] text-[#c7a15a]">Pipeline</p><h2 className="mt-3 font-serif text-2xl">Candidatures</h2>
          <div className="mt-7 grid gap-3 sm:grid-cols-2">{statusCounts.map(({status,count}) => <div key={status} className="border border-white/10 p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-white/40">{applicationLabels[status]}</p><p className="mt-2 font-serif text-2xl">{count}</p></div>)}</div>
