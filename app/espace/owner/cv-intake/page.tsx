    ...
    <section className="mx-auto w-[min(1280px,calc(100%-40px))] py-12 md:py-20">
      ...
      <div className="mt-8 border border-white/10 bg-[#111] p-6">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <input id="real-cv-file" type="file" accept=".pdf,.doc,.docx,.jpeg,.jpg,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,image/jpeg" onChange={(e) => setFile(e.target.files?.[0] || null)} className="sr-only" />\n          <label htmlFor="real-cv-file" className="flex min-h-12 cursor-pointer items-center justify-between gap-4 border border-white/10 bg-[#0b0b0b] px-4 py-3 text-sm text-white/70 hover:border-[#c7a15a] focus-within:border-[#c7a15a] md:col-span-3">\n            <span>{file ? file.name : "Sélectionner un CV réel (PDF, DOC, DOCX ou JPEG)"}</span>\n            <span className="shrink-0 border border-[#c7a15a] px-4 py-2 text-[10px] uppercase tracking-[0.14em] text-[#c7a15a]">Choisir un fichier</span>\n          </label>
