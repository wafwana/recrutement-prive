    ...
    <section className="mx-auto w-[min(1280px,calc(100%-40px))] py-12 md:w-[min(1280px,calc(100%-72px))] md:py-20">
      ...
      {selected ? (
        <section className="mt-8 border border-[#c7a15a]/40 bg-[#111] p-7">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] uppercase tracking-[0.22em] text-[#c7a15a]">{selected.sectorName}</p><h2 className="mt-2 font-serif text-3xl text-white">{selected.professionName}</h2><p className="mt-3 text-xs font-mono text-white/35">{selected.root}</p><div className="mt-5"><p className="text-[10px] uppercase tracking-[0.16em] text-[#F97316]">Profils forts demandés</p><div className="mt-3 flex flex-wrap gap-2">{getStrongProfilesForProfession(selectedSector, selectedProfession).map((profile) => <span key={profile.title} className="border border-[#F97316]/30 bg-[#F97316]/5 px-3 py-2 text-[10px] text-white/75">{profile.title}</span>)}</div></div></div><Link href="/espace/owner/metiers" className="border border-white/15 px-4 py-2 text-[10px] uppercase tracking-[0.15em] text-white/60">Fermer le dossier</Link></div>
        ...
        {RP_TAXONOMY.map((sector) => <section key={sector.code} className="border border-white/10 bg-[#111] p-6"><div className="flex flex-col gap-2 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] uppercase tracking-[0.22em] text-[#c7a15a]">SECTEUR</p><h2 className="mt-2 font-serif text-2xl text-white">{sector.name.fr}</h2><p className="mt-1 text-xs text-white/30">{sector.positioning.fr}</p></div><span className="text-[10px] uppercase tracking-[0.14em] text-white/30">{sector.subcategories.length} métiers / dérivés</span></div>
          <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {sector.subcategories.map((profession) => {
              ...
              return <Link key={profession.code} href={"/espace/owner/metiers?sector=" + encodeURIComponent(sector.code) + "&profession=" + encodeURIComponent(profession.code)} className={"border p-5 transition " + (isSelected ? "border-[#c7a15a] bg-[#c7a15a]/10" : "border-white/10 bg-[#0b0b0b] hover:border-white/25")}><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-white/80">{profession.name.fr}</p><p className="mt-2 text-[10px] font-mono text-white/25">{profession.code}</p></div><span className={"shrink-0 px-2 py-1 text-[9px] uppercase tracking-[0.12em] " + (count ? "bg-emerald-500/10 text-emerald-300" : "bg-white/5 text-white/30")}>{count} CV</span></div><div className="mt-4"><p className="text-[9px] uppercase tracking-[0.14em] text-[#F97316]">Profils forts demandés</p><div className="mt-2 flex flex-wrap gap-1.5">{getStrongProfilesForProfession(sector.code, profession.code).map((profile) => <span key={profile.title} className="border border-white/10 bg-white/[0.03] px-2 py-1 text-[9px] leading-4 text-white/60">{profile.title}</span>)}</div></div><p className="mt-4 text-[10px] uppercase tracking-[0.14em] text-[#c7a15a]">Ouvrir le dossier →</p></Link>;
            })}
          </div>
        </section>) }
      </div>
