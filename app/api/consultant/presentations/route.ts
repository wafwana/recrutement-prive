...
const bodySchema = z.object({
  missionId: z.string().min(1), applicationId: z.string().min(1), candidateId: z.string().min(1), companyId: z.string().min(1),
});
export async function POST(request: Request) {
  ...
  if (!(await hasPermission(session.user.id, role, "PRESENTATIONS_MANAGE"))) {
    return NextResponse.json({ error: "Permission de présentation non accordée par l'Owner." }, { status: 403 });
  }
  ...
  try {
    ...
    return NextResponse.json({ id: presentation.id, state: presentation.state, financialConditionStatus: presentation.financialConditionStatus, presentedAt: presentation.presentedAt }, { status: 201 });
  } catch (error) {
