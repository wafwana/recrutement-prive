    ...
    <section className="mx-auto w-[min(1180px,calc(100%-40px))] py-16 md:w-[min(1180px,calc(100%-72px))] md:py-24">
      ...
      <div className="mt-5 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-5 max-w-2xl text-sm leading-7 text-white/50">Vos informations et candidatures sont maintenant reliées à votre compte sécurisé.</p>
        </div>
        ...
        <aside className="space-y-8">
          ...
          <section className="border border-white/10 bg-[#111] p-8">
            ...
            <div className="mt-6 space-y-3">
              {profile?.cvUrl ? (
                <Link href={profile.cvUrl} target="_blank" className="block text-sm text-white/70 hover:text-white">CV principal ↗</Link>
              ) : null}
              {documents.map((document) => (
                <Link key={document.id} href={document.url} target="_blank" className="block text-sm text-white/70 hover:text-white">
                  {document.name} ↗
                </Link>
              ))}
