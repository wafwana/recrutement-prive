...
export default function OwnerConsultantsPage() {
  ...
  async function load() {
    ...
    setUsers((data.users || []).filter((user: StaffUser) => user.role === "CONSULTANT"));
    setLoading(false);
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      ...
      setMessage("Compte CONSULTANT créé par l'Owner.");
  ...
  async function handleAction(userId: string, action: "SUSPEND" | "REACTIVATE" | "REVOKE") {
    ...
    else { setMessage(`Statut mis à jour pour ${data.user.email}.`); void load(); }
  }
  ...
  return (
    <section className="mx-auto w-[min(900px,calc(100%-40px))] py-12 md:w-[min(900px,calc(100%-72px))] md:py-20">
      ...
      <h1 className="mt-4 font-serif text-4xl sm:text-5xl">Gestion des CONSULTANTS.</h1>
      <div className="mt-6 flex flex-wrap gap-3">
        <a href="/espace/owner/admins" className="inline-block border border-white/15 px-4 py-3 text-[10px] uppercase tracking-[0.18em] text-white/65">Gérer les administrateurs</a>
        <a href="/owner/permissions" className="inline-block border border-[#F97316] bg-[#F97316]/10 px-4 py-3 text-[10px] uppercase tracking-[0.18em] text-[#F97316]">Gestion des permissions</a>
      </div>
      <p className="mt-5 max-w-3xl text-sm leading-7 text-white/50">
        Seul l'Owner détient l'autorité suprême. Cette page gère exclusivement les consultants.
      </p>
          ...
          <form onSubmit={handleSubmit} className="mt-10 space-y-5 border border-white/10 bg-[#111] p-7">
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <label className="text-xs text-white/45 md:col-span-2">Mot de passe
                ...
                <span className="mt-2 block text-[11px] text-white/30">8+ caractères, majuscule, minuscule, chiffre et symbole.</span>
              </label>
            </div>
                ...
                <div key={u.id} className="flex flex-col gap-4 border border-white/10 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="border border-[#c7a15a]/30 px-2 py-0.5 text-[9px] uppercase tracking-widest text-[#c7a15a]">CONSULTANT</span>
                  ...
                  <div className="flex items-center gap-2">
                    {u.status === "ACTIVE" ? (
                      <button type="button" onClick={() => handleAction(u.id, "SUSPEND")} className="border border-amber-500/40 px-3 py-1.5 text-[10px] uppercase text-amber-300">Suspendre</button>
                    ) : (
                      <button type="button" onClick={() => handleAction(u.id, "REACTIVATE")} className="border border-emerald-500/40 px-3 py-1.5 text-[10px] uppercase text-emerald-300">Réactiver</button>
                    )}
                    <button type="button" onClick={() => handleAction(u.id, "REVOKE")} className="border border-red-500/40 px-3 py-1.5 text-[10px] uppercase text-red-300">Révoquer</button>
                  </div>
                </div>
              ))}
            </div>
