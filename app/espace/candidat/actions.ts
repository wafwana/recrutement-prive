...
export async function saveCandidateProfile(formData: FormData) {
  ...
  const parsed = profileSchema.safeParse({
    headline: asOptional(formData.get("headline")), bio: asOptional(formData.get("bio")), location: asOptional(formData.get("location")), country: asOptional(formData.get("country")), phone: asOptional(formData.get("phone")), phonePrefix: String(formData.get("phonePrefix") ?? "+33"), preferences: asOptional(formData.get("preferences")), skills: asOptional(formData.get("skills")), experienceYears: asOptional(formData.get("experienceYears")),
  });
  ...
  await prisma.candidateProfile.upsert({
    ...
    create: { userId: session.user.id, headline: parsed.data.headline, bio: parsed.data.bio, location: parsed.data.location, country: parsed.data.country, phonePrefix: parsed.data.phonePrefix, phone, preferences: csv(parsed.data.preferences), skills: csv(parsed.data.skills), experienceYears: parsed.data.experienceYears },
    update: { headline: parsed.data.headline, bio: parsed.data.bio, location: parsed.data.location, country: parsed.data.country, phonePrefix: parsed.data.phonePrefix, phone: phone ?? null, preferences: csv(parsed.data.preferences), skills: csv(parsed.data.skills), experienceYears: parsed.data.experienceYears ?? null },
  });
