...
test("adversarial check: financial payouts strictly require PENDING status and OWNER decision", () => {
  type Payout = { status: "PENDING" | "AUTHORIZED" | "REJECTED"; amountTtc: number; decisionNotes?: string };
...
test("adversarial check: payout decision 'METTRE EN ATTENTE' (PENDING) records reason and retains PENDING state for future OWNER decision", () => {
  type Payout = { id: string; status: "PENDING" | "AUTHORIZED" | "REJECTED"; decisionNotes?: string };

  function processOwnerDecision(payout: Payout, decision: "AUTHORIZED" | "REJECTED" | "PENDING", notes: string): Payout {
    assert.ok(notes.trim().length >= 5, "Reason required");
    if (payout.status !== "PENDING") {
      throw new Error("Already decided");
    }
    return { ...payout, status: decision, decisionNotes: notes };
  }

  const initialPayout: Payout = { id: "payout_1", status: "PENDING" };

  // Owner puts payout ON_HOLD ("METTRE EN ATTENTE") with notes
  const heldPayout = processOwnerDecision(initialPayout, "PENDING", "Analyse complémentaire du contrat requise");
  assert.equal(heldPayout.status, "PENDING");
  assert.equal(heldPayout.decisionNotes, "Analyse complémentaire du contrat requise");

  // Since status remains PENDING, Owner can subsequently authorize it
  const authorizedPayout = processOwnerDecision(heldPayout, "AUTHORIZED", "Contrat validé après vérification");
  assert.equal(authorizedPayout.status, "AUTHORIZED");
  assert.equal(authorizedPayout.decisionNotes, "Contrat validé après vérification");

  // Now that it's AUTHORIZED, further decisions are blocked
  assert.throws(() => processOwnerDecision(authorizedPayout, "REJECTED", "Tentative de modification tardive"), /Already decided/);
});
