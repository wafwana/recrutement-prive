...
export async function processOwnerRawOffer(formData:FormData){
 ...
 const analysis={...a,sourceType:"OWNER_RAW_OFFER",sourceUrl,outreach:{recipient,subject:a.outreachSubject,body:a.outreachBody,status:outreachStatus,sentAt,messageId}};
 await prisma.enterpriseSourcedOffer.upsert({where:{companyId_externalJobId:{companyId:company.id,externalJobId:external.id}},create:{companyId:company.id,externalJobId:external.id,selectedCountry:a.country||"",status:a.inPlatformScope?"QUALIFIED":"OUT_OF_SCOPE",analysis,matching,analyzedAt:now},update:{selectedCountry:a.country||"",status:a.inPlatformScope?"QUALIFIED":"OUT_OF_SCOPE",analysis,matching,analyzedAt:now}});
