...
export function matchCandidateToJob(candidate: MatchingCandidate, job: MatchingJob): MatchingResult {
  ...
  const normCandPrimary = candidate.primaryCategoryCode ? normalize(candidate.primaryCategoryCode) : null;
  const normJobCat = job.categoryCode ? normalize(job.categoryCode) : null;
  const normJobSubCat = job.subCategoryCode ? normalize(job.subCategoryCode) : null;

  const isSubCategoryMatch =
    Boolean(normJobSubCat) &&
    candSubCategories.has(normJobSubCat!) &&
    normJobSubCat !== normCandPrimary &&
    normJobSubCat !== normJobCat;

  if (isSubCategoryMatch) {
    categoryScore = 15;
    categoryMatchLevel = "EXACT_SUBCATEGORY";
  } else if (normJobCat && normCandPrimary && normCandPrimary === normJobCat) {
    categoryScore = 8;
