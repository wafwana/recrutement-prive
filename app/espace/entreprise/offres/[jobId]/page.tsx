...
export default async function CompanyJobPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  const companyJob = await prisma.job.findFirst({
    ...
    include: { applications: { where: { status: { in: companyVisibleStatuses } }, include: { candidate: { include: { user: { select: { name: true, email: true } }, documents: true } } }, orderBy: { updatedAt: "desc" } }, history: { orderBy: { createdAt: "desc" }, take: 30 } },
  });
