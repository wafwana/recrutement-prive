            ...
            {companyJob.applications.length === 0 ? <p className="text-sm text-white/45">Aucune candidature pour cette offre.</p> : companyJob.applications.map((application) => (
              <article key={application.id} className="border border-white/10 p-5">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="font-serif text-xl">{application.candidate.user.name || application.candidate.user.email}</h2>
                    <p className="mt-1 text-xs text-white/40">{application.candidate.headline || "Profil professionnel"}</p>
                  </div>
                  <span className="text-[10px] uppercase tracking-[0.16em] text-[#c7a15a]">{labels[application.status] ?? application.status}</span>
                </div>
                <p className="mt-4 text-sm leading-6 text-white/55">{application.candidate.bio || "Aucune présentation renseignée."}</p>
                <div className="mt-4 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.14em] text-white/35">
                  <span>{application.candidate.location || "Localisation inconnue"}</span>
                  <span>·</span>
                  <span>{application.candidate.documents.length} document(s)</span>
                </div>
              </article>
            ))}
          </div>
