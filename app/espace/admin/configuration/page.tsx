  ...
  return (
    <section className="mx-auto w-[min(1000px,calc(100%-40px))] py-14 md:w-[min(1000px,calc(100%-72px))] md:py-20">
      ...
      <div className="mt-10 space-y-3">
        {[
          ["maintenanceMode", "Mode maintenance", "Suspendre les nouveaux parcours lors d'une opération technique."],
          ["allowCandidateRegistration", "Inscription candidat", "Autoriser la création de nouveaux comptes candidats."],
          ["allowCompanyRegistration", "Inscription entreprise", "Autoriser la création de nouveaux comptes entreprise."],
          ["notificationsEnabled", "Notifications", "Activer les notifications applicatives futures."],
        ].map(([key, title, description]) => (
          <button key={key} type="button" onClick={() => toggle(key as keyof Settings)} className="flex w-full items-center justify-between gap-6 border border-white/10 bg-[#111] p-6 text-left transition hover:border-white/20">
            ...
            <span className={`h-6 w-11 rounded-full border p-1 ${settings[key as keyof Settings] ? "border-[#c7a15a]" : "border-white/20"}`}><span className={`block h-4 w-4 rounded-full transition ${settings[key as keyof Settings] ? "translate-x-5 bg-[#c7a15a]" : "bg-white/30"}`} /></span>
          </button>
        ))}
      </div>
